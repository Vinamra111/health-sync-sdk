...
const DEFAULT_OPTIONS = {
    ...
    minQuality: unified_data_1.DataQuality.LOW,
...
class DataNormalizer {
        ...
        for (const raw of rawData) {
            try {
                ...
                if (this.options.calculateQuality && this.options.minQuality) {
                    ...
                    if (!quality_scorer_1.QualityScorer.meetsThreshold({ quality: qualityScore }, this.options.minQuality)) {
    ...
    normalizeOne(raw) {
        ...
        switch (dataType) {
            case unified_data_1.DataType.STEPS:
                return this.normalizeSteps(raw, base);
            case unified_data_1.DataType.HEART_RATE:
            case unified_data_1.DataType.RESTING_HEART_RATE:
                return this.normalizeHeartRate(raw, base);
            case unified_data_1.DataType.SLEEP:
                return this.normalizeSleep(raw, base);
            case unified_data_1.DataType.ACTIVITY:
                return this.normalizeActivity(raw, base);
            case unified_data_1.DataType.CALORIES:
                return this.normalizeCalories(raw, base);
            case unified_data_1.DataType.DISTANCE:
                return this.normalizeDistance(raw, base);
            case unified_data_1.DataType.BLOOD_OXYGEN:
                return this.normalizeBloodOxygen(raw, base);
            case unified_data_1.DataType.BLOOD_PRESSURE:
                return this.normalizeBloodPressure(raw, base);
            case unified_data_1.DataType.WEIGHT:
                return this.normalizeWeight(raw, base);
            case unified_data_1.DataType.HEART_RATE_VARIABILITY:
                return this.normalizeHeartRateVariability(raw, base);
            case unified_data_1.DataType.VO2_MAX:
    ...
    inferDataType(raw) {
        ...
        if (sourceType.includes('step')) {
            return unified_data_1.DataType.STEPS;
        }
        if (sourceType.includes('heart') && sourceType.includes('rate')) {
            return unified_data_1.DataType.HEART_RATE;
        }
        if (sourceType.includes('sleep')) {
            return unified_data_1.DataType.SLEEP;
        }
        if (sourceType.includes('activity') || sourceType.includes('workout')) {
            return unified_data_1.DataType.ACTIVITY;
        }
        if (sourceType.includes('calorie')) {
            return unified_data_1.DataType.CALORIES;
        }
        if (sourceType.includes('distance')) {
            return unified_data_1.DataType.DISTANCE;
        }
        if (sourceType.includes('oxygen') || sourceType.includes('spo2')) {
            return unified_data_1.DataType.BLOOD_OXYGEN;
        }
        if (sourceType.includes('blood') && sourceType.includes('pressure')) {
            return unified_data_1.DataType.BLOOD_PRESSURE;
        }
        if (sourceType.includes('weight')) {
            return unified_data_1.DataType.WEIGHT;
        }
        if (sourceType.includes('hrv')) {
            return unified_data_1.DataType.HEART_RATE_VARIABILITY;
        }
        if (sourceType.includes('vo2')) {
            return unified_data_1.DataType.VO2_MAX;
        }
        // Default fallback - use raw.raw.type if available
        if (raw.raw['type']) {
            return raw.raw['type'];
        }
        throw new config_1.ValidationError(`Unable to infer data type from source type: ${raw.sourceDataType}`, ['sourceDataType']);
    }
    /**
    ...
    createMetadata(raw, dataType) {
        ...
        let quality = unified_data_1.DataQuality.MEDIUM;
        if (this.options.calculateQuality) {
            const qualityScore = quality_scorer_1.QualityScorer.calculateQualityScore({
                source: raw.source,
                dataType,
                manufacturer,
                model,
    ...
    normalizeSteps(raw, base) {
        ...
        if (this.options.validate) {
            const validation = validator_1.DataValidator.validateSteps(count);
            if (!validation.valid) {
                throw new config_1.ValidationError(`Invalid steps data: ${validation.errors.join(', ')}`, ['count']);
            }
        }
        const result = {
            ...base,
            dataType: unified_data_1.DataType.STEPS,
            count,
        };
    ...
    normalizeHeartRate(raw, base) {
        ...
        const context = raw.raw['context'] ?? unified_data_1.HeartRateContext.GENERAL;
        const minBpm = raw.raw['minBpm'] ? Number(raw.raw['minBpm']) : undefined;
        const maxBpm = raw.raw['maxBpm'] ? Number(raw.raw['maxBpm']) : undefined;
        if (this.options.validate) {
            const validation = validator_1.DataValidator.validateHeartRate(bpm);
            if (!validation.valid) {
                throw new config_1.ValidationError(`Invalid heart rate data: ${validation.errors.join(', ')}`, ['bpm']);
            }
        }
        const result = {
            ...base,
            dataType: unified_data_1.DataType.HEART_RATE,
            bpm,
            context,
        };
        if (minBpm !== undefined) {
    ...
    normalizeSleep(raw, base) {
        ...
        if (this.options.validate) {
            const validation = validator_1.DataValidator.validateSleepDuration(totalDuration);
            const efficiencyValidation = validator_1.DataValidator.validateSleepEfficiency(efficiency);
            if (!validation.valid) {
                throw new config_1.ValidationError(`Invalid sleep duration: ${validation.errors.join(', ')}`, ['totalDuration']);
            }
            if (!efficiencyValidation.valid) {
                throw new config_1.ValidationError(`Invalid sleep efficiency: ${efficiencyValidation.errors.join(', ')}`, ['efficiency']);
            }
        }
        const result = {
            ...base,
            dataType: unified_data_1.DataType.SLEEP,
            stages,
            totalDuration,
            efficiency,
        };
    ...
    normalizeActivity(raw, base) {
        const activityType = raw.raw['activityType'] ?? unified_data_1.ActivityType.OTHER;
        const duration = Number(raw.raw['duration'] ?? 0);
        const result = {
            ...base,
            dataType: unified_data_1.DataType.ACTIVITY,
            activityType,
            duration,
        };
        if (raw.raw['calories']) {
    ...
    normalizeCalories(raw, base) {
        ...
        if (this.options.validate) {
            const validation = validator_1.DataValidator.validateCalories(total);
            if (!validation.valid) {
                throw new config_1.ValidationError(`Invalid calories data: ${validation.errors.join(', ')}`, ['total']);
            }
        }
        const result = {
            ...base,
            dataType: unified_data_1.DataType.CALORIES,
            total,
        };
        if (raw.raw['active']) {
    ...
    normalizeDistance(raw, base) {
        ...
        if (unit === 'km' || unit === 'kilometers') {
            meters = unit_converter_1.UnitConverter.kmToMeters(meters);
        }
        else if (unit === 'miles') {
            meters = unit_converter_1.UnitConverter.milesToMeters(meters);
        }
        if (this.options.validate) {
            const validation = validator_1.DataValidator.validateDistance(meters);
            if (!validation.valid) {
                throw new config_1.ValidationError(`Invalid distance data: ${validation.errors.join(', ')}`, ['meters']);
            }
        }
        const result = {
            ...base,
            dataType: unified_data_1.DataType.DISTANCE,
            meters,
        };
    ...
    normalizeBloodOxygen(raw, base) {
        ...
        if (this.options.validate) {
            const validation = validator_1.DataValidator.validateBloodOxygen(percentage);
            if (!validation.valid) {
                throw new config_1.ValidationError(`Invalid blood oxygen data: ${validation.errors.join(', ')}`, ['percentage']);
            }
        }
        const result = {
            ...base,
            dataType: unified_data_1.DataType.BLOOD_OXYGEN,
            percentage,
        };
    ...
    normalizeBloodPressure(raw, base) {
        ...
        if (this.options.validate) {
            const validation = validator_1.DataValidator.validateBloodPressure(systolic, diastolic);
            if (!validation.valid) {
                throw new config_1.ValidationError(`Invalid blood pressure data: ${validation.errors.join(', ')}`, ['systolic', 'diastolic']);
            }
        }
        const result = {
            ...base,
            dataType: unified_data_1.DataType.BLOOD_PRESSURE,
            systolic,
            diastolic,
        };
    ...
    normalizeWeight(raw, base) {
        ...
        if (unit === 'lbs' || unit === 'pounds') {
            kilograms = unit_converter_1.UnitConverter.poundsToKg(kilograms);
        }
        if (this.options.validate) {
            const validation = validator_1.DataValidator.validateWeight(kilograms);
            if (!validation.valid) {
                throw new config_1.ValidationError(`Invalid weight data: ${validation.errors.join(', ')}`, ['kilograms']);
            }
        }
        const result = {
            ...base,
            dataType: unified_data_1.DataType.WEIGHT,
            kilograms,
        };
        // Always include pounds conversion for convenience
        const pounds = unit_converter_1.UnitConverter.kgToPounds(kilograms);
        if (pounds !== undefined) {
    ...
    normalizeHeartRateVariability(raw, base) {
        ...
        const result = {
            ...base,
            dataType: unified_data_1.DataType.HEART_RATE_VARIABILITY,
            milliseconds,
        };
    ...
    normalizeVO2Max(raw, base) {
        ...
        const result = {
            ...base,
            dataType: unified_data_1.DataType.VO2_MAX,
            value,
        };
        const method = raw.raw['method'];
        if (method !== undefined) {
