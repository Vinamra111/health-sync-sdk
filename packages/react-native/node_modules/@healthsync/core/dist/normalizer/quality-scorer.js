...
class QualityScorer {
    ...
    static calculateQualityScore(params) {
        const { source, dataType, manufacturer, isManualEntry = false, recordedAt, syncedAt, completeness = 1.0, } = params;
        ...
        let score = sourceReliability * weights.source +
     ...
     * @private
     */
    static getDeviceAccuracy(manufacturer, dataType) {
        if (!manufacturer) {
            return 0.6;
        }
     ...
     * @private
     */
    static calculateFreshness(recordedAt, _syncedAt) {
        if (!recordedAt) {
    ...
    static scoreToQuality(score) {
        if (score >= 80) {
            return unified_data_1.DataQuality.HIGH;
        }
        else if (score >= 60) {
            return unified_data_1.DataQuality.MEDIUM;
        }
        else if (score >= 40) {
            return unified_data_1.DataQuality.LOW;
        }
        else {
            return unified_data_1.DataQuality.UNKNOWN;
        }
    }
    /**
     ...
     */
    static meetsThreshold(score, minQuality) {
        const qualityOrder = [
            unified_data_1.DataQuality.UNKNOWN,
            unified_data_1.DataQuality.LOW,
            unified_data_1.DataQuality.MEDIUM,
            unified_data_1.DataQuality.HIGH,
        ];
 ...
 */
Object.defineProperty(QualityScorer, "SOURCE_RELIABILITY", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: {
        [unified_data_1.HealthSource.HEALTH_CONNECT]: 0.8, // Aggregated data, generally reliable
        [unified_data_1.HealthSource.APPLE_HEALTH]: 0.85, // Apple's quality control
        [unified_data_1.HealthSource.FITBIT]: 0.75, // Consumer device
        [unified_data_1.HealthSource.GARMIN]: 0.8, // Sports/fitness focused
        [unified_data_1.HealthSource.OURA]: 0.85, // High-quality sleep tracking
        [unified_data_1.HealthSource.WHOOP]: 0.85, // Professional athlete-grade
        [unified_data_1.HealthSource.STRAVA]: 0.7, // Activity tracking
        [unified_data_1.HealthSource.MYFITNESSPAL]: 0.6, // Self-reported nutrition
        [unified_data_1.HealthSource.UNKNOWN]: 0.5, // Unknown source
    }
});
/**
 * Device manufacturer accuracy ratings
 */
Object.defineProperty(QualityScorer, "DEVICE_ACCURACY", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: {
