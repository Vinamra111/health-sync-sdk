...
exports.HealthConnectPlugin = void 0;
const plugin_interface_1 = require("../plugin-interface");
const plugin_interface_2 = require("../plugin-interface");
 ...
 * @extends {BasePlugin}
 */
class HealthConnectPlugin extends plugin_interface_1.BasePlugin {
    /**
    ...
    constructor(config = {}) {
        ...
        Object.defineProperty(this, "supportedDataTypes", {
            ...
            writable: true,
            value: [
                unified_data_1.DataType.STEPS,
                unified_data_1.DataType.HEART_RATE,
                unified_data_1.DataType.RESTING_HEART_RATE,
                unified_data_1.DataType.SLEEP,
                unified_data_1.DataType.ACTIVITY,
                unified_data_1.DataType.CALORIES,
                unified_data_1.DataType.DISTANCE,
                unified_data_1.DataType.BLOOD_OXYGEN,
                unified_data_1.DataType.BLOOD_PRESSURE,
                unified_data_1.DataType.BODY_TEMPERATURE,
                unified_data_1.DataType.WEIGHT,
                unified_data_1.DataType.HEIGHT,
                unified_data_1.DataType.HEART_RATE_VARIABILITY,
            ]
        });
        ...
        this.hcConfig = {
            ...types_1.DEFAULT_HEALTH_CONNECT_CONFIG,
            ...config,
        };
    }
    /**
    ...
    async initialize(config) {
        ...
        if (availability !== types_1.HealthConnectAvailability.INSTALLED) {
            throw new config_1.ConnectionError(`Health Connect is ${availability}. Please install Health Connect from Google Play Store.`, unified_data_1.HealthSource.HEALTH_CONNECT, undefined, { code: 'HEALTH_CONNECT_NOT_AVAILABLE', availability });
        }
    ...
    async dispose() {
        ...
        this.connectionStatus = plugin_interface_2.ConnectionStatus.DISCONNECTED;
    ...
    async connect() {
        ...
        try {
            ...
            const availability = await this.checkAvailability();
            if (availability !== types_1.HealthConnectAvailability.INSTALLED) {
                this.connectionStatus = plugin_interface_2.ConnectionStatus.ERROR;
                return {
                    success: false,
                    message: `Health Connect is ${availability}`,
                    metadata: {
                        custom: {
                            source: unified_data_1.HealthSource.HEALTH_CONNECT,
                            availability,
                        },
                    },
                };
            ...
            this.connectionStatus = plugin_interface_2.ConnectionStatus.CONNECTED;
    ...
    async fetchData(query) {
        if (this.connectionStatus !== plugin_interface_2.ConnectionStatus.CONNECTED) {
            throw new config_1.ConnectionError('Not connected to Health Connect', unified_data_1.HealthSource.HEALTH_CONNECT, undefined, { code: 'NOT_CONNECTED' });
        }
        this.log('info', `Fetching ${query.dataType} data from ${query.startDate} to ${query.endDate}`);
        // Check if data type is supported
        const typeInfo = types_1.HEALTH_CONNECT_TYPE_MAP[query.dataType];
        if (!typeInfo || !typeInfo.recordType) {
            throw new config_1.DataFetchError(`Data type ${query.dataType} is not supported by Health Connect`, unified_data_1.HealthSource.HEALTH_CONNECT, query.dataType, undefined, { code: 'UNSUPPORTED_DATA_TYPE' });
        }
        ...
        if (!hasPermission) {
            throw new config_1.AuthenticationError(`Missing permissions for ${query.dataType}`, undefined, { code: 'MISSING_PERMISSIONS', dataType: query.dataType, source: unified_data_1.HealthSource.HEALTH_CONNECT });
        }
        try {
            const readRequest = {
            ...
            throw new config_1.DataFetchError(`Failed to fetch ${query.dataType} data: ${error.message}`, unified_data_1.HealthSource.HEALTH_CONNECT, query.dataType, undefined, { code: 'FETCH_FAILED', originalError: error });
        }
    }
    /**
    ...
    handleError(error) {
        if (error.message.includes('permission')) {
            return plugin_interface_2.ErrorAction.RETRY;
        }
        ...
        return plugin_interface_2.ErrorAction.FAIL;
    }
    // ============================================================================
    // Helper Methods
    // ============================================================================
    ...
    getAllRequiredPermissions() {
        ...
        this.supportedDataTypes.forEach((dataType) => {
            const typeInfo = types_1.HEALTH_CONNECT_TYPE_MAP[dataType];
            if (typeInfo) {
