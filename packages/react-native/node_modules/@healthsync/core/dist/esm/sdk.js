...
import { ConnectionStatus, } from './plugins/plugin-interface';
import { PluginRegistry } from './plugins/plugin-registry';
import { DEFAULT_SDK_CONFIG, DEFAULT_SYNC_OPTIONS, ConnectionError, SyncError, LogLevel, } from './types/config';
import { SDKEvent, } from './types/query';
...
export class HealthSyncSDK {
    ...
    async connect(source) {
        ...
        if (!plugin) {
            throw new ConnectionError(`No plugin registered for source: ${source}`, source, 404);
        }
        try {
            ...
            this.log(LogLevel.ERROR, `Error connecting to ${source}`, error);
            throw new ConnectionError(`Failed to connect to ${source}: ${error.message}`, source, 500, { originalError: error });
        }
    }
    /**
    ...
    async disconnect(source) {
        ...
        try {
            ...
            this.log(LogLevel.ERROR, `Error disconnecting from ${source}`, error);
            throw new ConnectionError(`Failed to disconnect from ${source}: ${error.message}`, source, 500, { originalError: error });
        }
    }
    /**
        ...
        for (const source of sources) {
            ...
            try {
                ...
                if (normalizationResult.warnings.length > 0) {
                    this.log(LogLevel.WARN, `Normalization warnings for ${source}: ${normalizationResult.warnings.join(', ')}`);
                }
            }
            catch (error) {
                this.log(LogLevel.ERROR, `Error querying ${source}`, error);
                sourcesFailed.push(source);
                errors.push(new SyncError(`Failed to query ${source}: ${error.message}`, 'fetch', source, request.dataType));
            }
        }
        ...
        for (const source of sources) {
            ...
            for (const dataType of dataTypes) {
                try {
                    ...
                    errors.push(new SyncError(`Sync failed for ${source} - ${dataType}`, 'fetch', source, dataType));
                }
            }
            sourceResults.push({
                source,
