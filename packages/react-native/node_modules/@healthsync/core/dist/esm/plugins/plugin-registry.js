...
import { ConnectionStatus } from './plugin-interface';
...
export class PluginRegistry {
    ...
    register(plugin) {
        if (this.plugins.has(plugin.id)) {
            throw new PluginError(`Plugin with ID '${plugin.id}' is already registered`, plugin.id, 400);
        }
        // Validate plugin implements required properties
        if (!plugin.id || !plugin.name || !plugin.version) {
            throw new ConfigurationError('Plugin must have id, name, and version properties', { pluginId: plugin.id });
        }
        const entry = {
            plugin,
            initialized: false,
